...
var _streamyDom = __webpack_require__(12);
...
function render(vdom, parentElement) {
  ...
  if (!(0, _streamy.isStream)(vdom$)) vdom$ = (0, _streamy.stream)(vdom$);
...
*/
var h = exports.h = function h(hyperscriptTag, props) {
  ...
  var elementConstructor = function elementConstructor(globals) {
    ...
    if (typeof hyperscriptTag === "function") {
      ...
      var output = hyperscriptTag(props, mergedChildren$, globals);
    ...
    var _resolveTag = resolveTag(hyperscriptTag),
        tag = _resolveTag.tag,
        classes = _resolveTag.classes,
        id = _resolveTag.id;
...
*/
function resolveTag(hyperscriptTag) {
  var id = void 0;
  var classes = [];
  var tag = void 0;
  while (hyperscriptTag.length > 0) {
    var end = indexOfRegex(hyperscriptTag.substr(1), /[\.#]/); // ignore first
    ...
    if (hyperscriptTag.startsWith(".")) {
      ...
      if (id) throw Error("You have set two ids in the selector " + hyperscriptTag);
  ...
  return { id: id, classes: classes, tag: tag || "div" };
}
...
function mergeHyperscript(props, id, classes) {
  ...
  // add id
  if (props.id && id) throw Error("You defined two ids [" + props.id + "," + id + "] on one element.");
  if (id) props.id = id;

  // add classes
  if (classes.length > 0) {
    if (props.class) props.class = (0, _streamy.merge$)([props.class, classes.join(" ")]).map(function (classResults) {
      return classResults.join(" ");
