  ...
  schedule = schedule.map(function (expected) {
    if (typeof expected === "string") {
      return function (_ref) {
        ...
        return expect(element.outerHTML).toBe(expected);
      };
    }
    return expected;
  });
  ...
  function handleTemplateLiteral(globals, strings, values) {
    ...
    for (var _i = 0; _i < parts.length; _i++) {
      ...
      if (tokenType === OPEN && /^\//.test(p[1])) {
        ...
        if (stack.length > 1) {
          stack.pop();
          stack[stack.length - 1][0][2][ix] = h(cur[0], cur[1], cur[2].length ? trimWhitespacesInChildren(cur[0], cur[2]) : undefined);
        }
    ...
    if (Array.isArray(tree[2][0]) && typeof tree[2][0][0] === "string" && Array.isArray(tree[2][0][2])) {
      // trim whitespaces
      var children = trimWhitespacesInChildren(tree[2][0][0], tree[2][0][2]);
...
function trimWhitespacesInChildren(tag, children) {
  ...
  while (copy && typeof copy[copy.length - 1] === "string" && /^\s*$/.test(copy[copy.length - 1])) {
...
function resolveInputToStream(input, globals) {
  ...
  if ((0, _streamy.isStream)(input)) {
    vdom$ = input;
  } else if (input instanceof _streamyVdom.Component) {
  ...
  if (!(0, _streamy.isStream)(vdom$)) vdom$ = resolveInputToStream(vdom$);
