...
var h = exports.h = function h(tag, props) {
	...
	if (typeof tag === 'function') {
		elem = tag(addStreamDetacher(props, deleted$), addStreamDetacher(children, deleted$), deleted$);
	} else {
		elem = (0, _streamyDom.createElement)(tag, wrapProps$(props, deleted$), makeChildrenStreams$(children, deleted$));
	}

	// ATTENTION: the stream breaking is not working for returned arrays currently
	if (elem.addEventListener !== undefined) {
		elem.addEventListener(_streamyDom.REMOVED, function () {
			return deleted$(true);
		});
		elem.addEventListener(_streamyDom.ADDED, function () {
