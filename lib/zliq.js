...
function until(parent$, stopEmitValues$) {
	...
	var subscribeTo = function subscribeTo(stream) {
		return stream.listeners.push(newStream);
	};
...
var h = exports.h = function h(tag, props) {
	...
	if (typeof tag === 'function') {
		...
		return tag(props, children, deleted$);
	}
	var elem = (0, _streamyDom.createElement)(tag, wrapProps$(props, deleted$), makeChildrenStreams$(children, deleted$));

	elem.addEventListener('REMOVED', function () {
		return deleted$(true);
	});
	elem.addEventListener('ADDED', function () {
