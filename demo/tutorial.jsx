...
export const Tutorial = ({router$}, children, globals) =>
    <div class="section tutorial">
        ...
        <p>The format of the stream as a function makes it easy to pipe events into streams.</p>
        <Markup>
            {`
        ...
        <p>There are a bunch of stream manipulation functions available: <code>.map</code>, <code>.is</code>, <code>.filter</code>, <code>.distinct</code> and <code>.reduce</code>. Checkout <code>src/utils/streamy.js</code> for descriptions.</p>
        ...
        <p>The counterpart is the <code>.patch</code> functions. It will assume the stream to be a state object and it updates just parts of the object:</p>
        ...
        <p>ZLIQ streams are always hot, meaning they will send their last value on hooking into it. BUT the streams will not emit `undefined`!</p>
        ...
        <p>For a component based on state like used in the most MV* frameworks just define a state stream locally.</p>
        ...
        <p>ZLIQ integrates lifecycle event. You can add an attribute cycle to any component holding functions for the lifecycle events `created`, `mounted`, `updated` and `removed`. This way you can perform actions like initialization jQuery plugins on the element.</p>
